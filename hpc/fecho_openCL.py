...
#x_np = (np.array([-3,-1,2.5,-1,1,1,-2,1.5,2])).astype(np.float32)
#y_np = (np.array([1.5,0,2.5,4,-1,-2,-2,2,-2])).astype(np.float32)

random_numbers_x = np.random.uniform(0,10,size=n).tolist()
random_numbers_y = np.random.uniform(0,10,size=n).tolist()

x_np = (np.array(random_numbers_x)).astype(np.float32)
y_np = (np.array(random_numbers_y)).astype(np.float32)

#x_np = (np.array([-3,-1,2.5])).astype(np.float32)
#y_np = (np.array([1.5,0,2.5])).astype(np.float32)



#pegar um ponto da lista de menor ordenada e menor abiscissa
id_p = None
menor_y = y_np[0]
menor_x = x_np[0]
...
#plt.plot(menor_x, menor_y, 'ro') 
plt.axis([min(x_np)-abs(min(x_np)/10), max(x_np)+abs(max(x_np)/10), min(y_np)-abs(min(y_np)/10), max(y_np)+abs(max(y_np)/10)])
plt.show()
